...
def check_gradient(f, x, delta=1e-5, tol = 1e-4):
    ...
    print("analytic_grad.shape: " + str(analytic_grad.shape))
    ...
    while not it.finished:
        ...
        fx_first = f(first_x)
        fx_second = f(second_x)
    
        # DONE compute value of numeric gradient of f to idx
        fx_first, analytic_grad_first = f(first_x)
        fx_second, analytic_grad_second = f(second_x)
        numeric_grad_at_ix = (fx_first - fx_second) / (2 * delta)
